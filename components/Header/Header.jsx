...
export default function Header(props) {
  const { bgColor = "", user = false } = props;
  ...
  return (
    <header className={`${styles.header} ${bgColor === "white" ? styles.whiteBg : ""}`}>
      <div className={styles.container}>
        <Link href="/" className={`${styles.logo} ${bgColor === "white" ? styles.whiteBg : ""}`}>
        ...
        <div className={styles.rightSection}>
          ...
          <div className={`${styles.actions} ${user ? styles.hasUser : ""}`}>
            {user ? (
              <div className={`${styles.userProfile} ${bgColor === "white" ? styles.whiteBg : ""}`}>
                <div className={styles.avatarWrapper}>
                  <Image src={girlImage} alt="avatar" width={63} height={63} className={styles.avatarImage} />
                </div>
                <span className={styles.userName}>Lina</span>
                <svg width="12" height="8" viewBox="0 0 12 8" className={styles.chevron}>
            ...
            <button
              ref={buttonRef}
              className={`${styles.menuButton} ${bgColor=="white"?styles.whiteBgBurger:""} ${isOpen ? styles.active : ""}`}
      ...
      {isOpen && (
        <div className={styles.mobileMenu}>
          <ul className={styles.mobileNav}>
            ...
            <li><Link href="/membership" onClick={() => setIsOpen(false)}>About Us</Link></li>
            <li><Link href="/course" onClick={() => setIsOpen(false)}>Courses</Link></li>
